 ...
 * @author zhangbo
 */
public class OwnEventBusClient {} /*{
    ...
    *//**
     * Creates an {@code OwnEventBusClient} instance to connect to a Vert.x EventBus TCP bridge with default options.
     *
     * @return the bus client
     *//*
    public static OwnEventBusClient tcp() {
        return OwnEventBusClient.tcp(new EventBusClientOptions(), defaultCodecOrDie());
    }

    *//**
     * Creates an {@code OwnEventBusClient} instance to connect to a Vert.x EventBus TCP bridge.
     *
     * @param options the {@code OwnEventBusClient} options
     * @return the bus client
     *//*
    public static OwnEventBusClient tcp(EventBusClientOptions options) {
        return OwnEventBusClient.tcp(options, defaultCodecOrDie());
    }
    ...
    *//**
     ...
     * @return the bus client
     *//*
    public static OwnEventBusClient tcp(EventBusClientOptions options, JsonCodec codec) {
        options = new EventBusClientOptions(options);
        if (options.getPort() == -1) {
            options.setPort(7000);
        }
        return new OwnEventBusClient(new TcpTransport(options), options, codec);
    }

    *//**
     * Creates an {@code OwnEventBusClient} instance to connect to a Vert.x EventBus SockJS bridge using WebSockets
     * with default options.
     *
     * @return the bus client
     *//*
    public static OwnEventBusClient webSocket() {
        return OwnEventBusClient.webSocket(new EventBusClientOptions(), defaultCodecOrDie());
    }
    ...
    *//**
     ...
     * @return the bus client
     *//*
    public static OwnEventBusClient webSocket(JsonCodec codec) {
        return OwnEventBusClient.webSocket(new EventBusClientOptions(), codec);
    }
    ...
    *//**
     ...
     * @return the bus client
     *//*
    public static OwnEventBusClient webSocket(EventBusClientOptions options) {
        return OwnEventBusClient.webSocket(options, defaultCodecOrDie());
    }
    ...
    *//**
     ...
     * @return the bus client
     *//*
    public static OwnEventBusClient webSocket(EventBusClientOptions options, JsonCodec codec) {
        options = new EventBusClientOptions(options);
        if (options.getPort() == -1) {
            options.setPort(80);
        }
        return new OwnEventBusClient(new WebSocketTransport(options), options, codec);
    }
    ...
    private final ConcurrentMap<String, OwnEventBusClient.HandlerList> consumerMap = new ConcurrentHashMap<String, OwnEventBusClient.HandlerList>();
    ...
    public OwnEventBusClient(Transport transport, EventBusClientOptions options, JsonCodec codec) {
        this.transport = transport;
        this.bootstrap = new Bootstrap().group(this.group);
        this.options = options;
        this.codec = codec;
        this.logger = InternalLoggerFactory.getInstance(OwnEventBusClient.class);
    }
    ...
    private synchronized void execute(Handler<Transport> task) {
        if (connected) {
            ...
            logger.error("This OwnEventBusClient is closed.");
        } else {
    ...
    private synchronized void initializeTransport() {
        ...
        transport.connectedHandler(new Handler<Void>() {
            ...
            public void handle(Void v) {
                synchronized (OwnEventBusClient.this) {
                    ...
                    pingPeriodic = group.next().scheduleAtFixedRate(new Runnable() {
                            ...
                            OwnEventBusClient.this.options.getPingInterval(),
                            OwnEventBusClient.this.options.getPingInterval(),
                    ...
                    if (OwnEventBusClient.this.connectedHandler != null) {

                        OwnEventBusClient.this.connectedHandler.handle(new Handler<Void>() {
                            @Override
                            public void handle(Void v) {
                                OwnEventBusClient.this.handlePendingTasks();
                            }
                        });
                    } else {
                        OwnEventBusClient.this.handlePendingTasks();
    ...
    private synchronized void connectTransport() {
        ...
        String host = OwnEventBusClient.this.options.getHost();
        int port = OwnEventBusClient.this.options.getPort();

        if (OwnEventBusClient.this.options.getProxyHost() != null) {
            logger.info("Connecting to bridge at " + host + ":" + port + " (via " + OwnEventBusClient.this.options.getProxyHost() + ") using " + this.transport.getClass().getSimpleName() + "...");
        } else {
    ...
    private synchronized void autoReconnect() {
        ...
        if (!closed &&
                ...
                OwnEventBusClient.this.options.isAutoReconnect() &&
                (OwnEventBusClient.this.options.getMaxAutoReconnectTries() == 0 ||
                        reconnectTries < OwnEventBusClient.this.options.getMaxAutoReconnectTries())) {
            ++reconnectTries;
            int interval = OwnEventBusClient.this.options.getAutoReconnectInterval();
    ...
    private void handleMsg(Map msg) {
        ...
        if (type != null) {
            if ("message".equals(type) || "rec".equals(type)) {
                ...
                OwnEventBusClient.HandlerList consumers = consumerMap.get(address);
                if (consumers != null) {
...
//          String failureType = msg.get("failureType").getAsString();
    ...
    *//**
     ...
     *//*
    public OwnEventBusClient setDefaultDeliveryOptions(DeliveryOptions defaultOptions) {
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public OwnEventBusClient connect() {
        closed = false;
        initializeTransport();
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public OwnEventBusClient send(String address, Object message) {
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public <T> OwnEventBusClient request(String address, Object message, final Handler<AsyncResult<Message<T>>> replyHandler) {
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public OwnEventBusClient send(String address, Object message, DeliveryOptions options) {
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public <T> OwnEventBusClient request(String address, Object message, DeliveryOptions options, final Handler<AsyncResult<Message<T>>> replyHandler) {
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public OwnEventBusClient publish(String address, Object message) {
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public OwnEventBusClient publish(String address, Object message, DeliveryOptions options) {
    ...
    *//**
    ...
    private void register(MessageHandler<?> handler, Map<String, String> headers, boolean atServer) {
        ...
        synchronized (consumerMap) {
            ...
            consumerMap.put(address, new OwnEventBusClient.HandlerList(handlers, atServer));
        }
    }
    ...
    void unregister(MessageHandler<?> handler, boolean atServer) {
        ...
        synchronized (consumerMap) {
            ...
            if (atServer && handlers.isEmpty()) {
                ...
                consumerMap.put(address, new OwnEventBusClient.HandlerList(new ArrayList<MessageHandler>(handlers), atServer));
            }
        }
    }
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public OwnEventBusClient connectedHandler(Handler<Handler<Void>> connectedHandler) {
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public OwnEventBusClient exceptionHandler(Handler<Throwable> exceptionHandler) {
    ...
    *//**
     ...
     * @return a reference to this, so the API can be used fluently
     *//*
    public synchronized OwnEventBusClient closeHandler(Handler<Void> closeHandler) {
